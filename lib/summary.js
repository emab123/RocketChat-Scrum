...
async function sendImmediate(g) {
    ...
    let immediates = g.completedResponses;
    if(immediates.length === 0)
        return;
    //send
    for(let response of immediates) {
        let msg = rocket.driver.prepareMessage(getMessage(messageList.LATE_SUMMARY, response), group._id);
        msg.attachments = [];
        for(let i=0; i < 3; i++) {
            msg.attachments.push({
                title: getMessage(stageToMessage(i), response),
                fields: [{
                    short: false,
...
async function sendSummary(group) {
    ...
    let completed = group.completedResponses;
    ...
    for(let i=0; i<3; i++) {
        mes.attachments.push({
            ...
            fields: [{
                short: false,
                value: normals.map(e => `> **${e.user}**: ${e.messages[i]}`).join("\n")
            }]
        });
    }
...
exports.setup = async function(register = true) {
    const fn = async () => {
        ...
        try {
            ...
            logger.error("Summary has thrown the following error");
            logger.error(e);
        } finally {
            running = false
        }
